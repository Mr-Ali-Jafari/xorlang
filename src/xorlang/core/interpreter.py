...
# XorLang Interpreter
# Tree-walking interpreter for executing XorLang AST nodes
...
from .errors import RuntimeError, ReturnSignal
...
class Interpreter:
    """Tree-walking interpreter for XorLang."""
    ...
    def _install_builtins(self) -> None:
        ...
        if self.stdlib_path and os.path.isdir(self.stdlib_path):
            self._load_stdlib_file("prelude.xor")
            self._load_stdlib_file("gui.xor")
    ...
    def _create_builtin_array_class(self):
        ...
        def array_push(this, item):
            get_native_list(this).append(item)
        ...
        def array_set(this, index, value):
            ...
            try:
                ...
                if 0 <= idx < len(items):
                    items[idx] = value
                # Optionally handle out-of-bounds set, e.g., by extending the list
            except (ValueError, TypeError):
                raise RuntimeError(f"Array index must be an integer, got {index}")
        ...
        def array_index_of(this, item):
            try:
                return get_native_list(this).index(item)
            except ValueError:
                return -1
    ...
    def _load_stdlib_file(self, filename):
        """Finds, parses, and evaluates a standard library file."""
        ...
        if lib_path and os.path.exists(lib_path):
            try:
                ...
                if error:
                    # We can't use the error_handler here easily, so just print
                    print(f"Lexer Error in {filename}: {error}")
                    return

                parser = Parser(tokens)
                ast = parser.parse()
                if ast.error:
                    print(f"Parse Error in {filename}: {ast.error}")
                    return

                if ast.node:
                    self.eval(ast.node) # Evaluate in global scope
    ...
    def eval(self, node: ASTNode, env: Optional[Environment] = None) -> Any:
        ...
        if node_type_name == 'VarDeclNode':
            return self._eval_VarDeclNode(node, env)
        if node_type_name == 'AssignNode':
            return self._eval_AssignNode(node, env)
        if node_type_name == 'ThisNode':
            return self._eval_ThisNode(node, env)

        method_name = f'_eval_{node_type_name}'
    ...
    def _eval_MemberAccessNode(self, node: MemberAccessNode, env: Environment) -> Any:
        """Evaluate member access (object.member)."""
        ...
        if isinstance(obj, InstanceValue):
            ...
            if member_name in obj.members:
                return obj.members[member_name]
            else:
